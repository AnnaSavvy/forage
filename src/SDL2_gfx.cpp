...
Addiotional modifications (C) 2023 idshibanov
...
#error lrint needed for MSVC on non X86/AMD64/ARM targets.
    ...
    */
    ...
    \returns Returns 0 on success, -1 on failure.
    */
    int filledPolygonRGBAMT( SDL_Renderer * renderer, const Sint16 * vx, const Sint16 * vy, int n, Uint8 r, Uint8 g, Uint8 b, Uint8 a, int ** polyInts,
                             int * polyAllocated )
    {
        int result;
        int i;
    ...
    */
    ...
    \returns Returns 0 on success, -1 on failure.
    */
    int texturedPolygonMT( SDL_Renderer * renderer, const Sint16 * vx, const Sint16 * vy, int n, SDL_Surface * texture, int texture_dx, int texture_dy, int ** polyInts,
                           int * polyAllocated )
    {
        int result;
        int i;
    ...
    */
    ...
    static void x_perpendicular( SDL_Renderer * B, int x0, int y0, int dx, int dy, int xstep, int ystep, int einit, int w_left, int w_right, int winit )
    {
    ...
    static void x_varthick_line( SDL_Renderer * B, int style, int x0, int y0, int dx, int dy, int xstep, int ystep, double thickness, int pxstep, int pystep )
    {
        ...
        for ( p = 0; p < length; p++ ) {
            ...
            if ( style < 0 )
                x_perpendicular( B, x, y, dx, dy, pxstep, pystep, p_error, w_left, w_right, error );
            if ( error >= threshold ) {
                y = y + ystep;
                error = error + E_diag;
                if ( p_error >= threshold ) {
                    if ( style < 0 )
                        x_perpendicular( B, x, y, dx, dy, pxstep, pystep, ( p_error + E_diag + E_square ), w_left, w_right, error );
    ...
    static void y_perpendicular( SDL_Renderer * B, int x0, int y0, int dx, int dy, int xstep, int ystep, int einit, int w_left, int w_right, int winit )
    {
        int x, y, threshold, E_diag, E_square;
        int tk;
        int error;
    ...
    static void y_varthick_line( SDL_Renderer * B, int style, int x0, int y0, int dx, int dy, int xstep, int ystep, double thickness, int pxstep, int pystep )
    {
        int p_error, error, x, y, threshold, E_diag, E_square, length, p;
        int w_left, w_right;
        double D;
        ...
        for ( p = 0; p < length; p++ ) {
            ...
            if ( error >= threshold ) {
                ...
                if ( p_error >= threshold ) {
                    if ( style < 0 )
                        y_perpendicular( B, x, y, dx, dy, pxstep, pystep, p_error + E_diag + E_square, w_left, w_right, error );
                    p_error = p_error + E_diag;
                }
                p_error = p_error + E_square;
            }
    ...
    void draw_varthick_line( SDL_Renderer * B, int style, int x0, int y0, int x1, int y1, double thickness )
    {
        ...
        if ( dx > dy )
            x_varthick_line( B, style, x0, y0, dx, dy, xstep, ystep, thickness + 1.0, pxstep, pystep );
        else
            y_varthick_line( B, style, x0, y0, dx, dy, xstep, ystep, thickness + 1.0, pxstep, pystep );
        return;
    }
    ...
    */
    ...
    int thickLineColorStyle( SDL_Renderer * renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint8 width, Uint32 color, int style )
    {
        Uint8 * c = (Uint8 *)&color;
        LineStyle = style;
        return thickLineRGBA( renderer, x1, y1, x2, y2, width, c[0], c[1], c[2], c[3] );
